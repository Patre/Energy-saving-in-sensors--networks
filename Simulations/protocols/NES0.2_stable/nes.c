/**
 *  \file   nes.c
 *  \brief  Neighbor elimination scheme protocol
...
int setnode(call_t *c, void *params) {
    ...
    nodedata->Voi_NES   = Nullptr(list);
    nodedata->paquets   = Nullptr(packet_NES);
...
int init(call_t *c, void *params) {
    struct entitydataNES *entitydata = malloc(sizeof(struct entitydataNES));
    param_t *param;
...
int bootstrap(call_t *c) {
    ...
    scheduler_add_callback(c->node*time_seconds_to_nanos(nodedata->eps), c, init_one_hop, NULL);
...
void rx(call_t *c, packet_t *packet) {
    ...
    if(data->type == HELLO)
    {
        rx_one_hope(c,packet);
    }
    else if(data->type == REP_HELLO)
    {
        rx_one_hope_reponse(c,packet);
    }
    /*******************************NES***************************/
    else if(data->type == NES)
    {
        rx_nes(c,packet);
    }
...
int unsetnode(call_t *c) {
    ...
    /*int i=0;
    printf("%d:{ ",c->node);
    for(i=0;i<list_taille(nodedata->N1);i++)    printf("%d,",list_get(nodedata->N1,i));
    printf("} nbrv: %d\n",list_taille(nodedata->N1));//*/

    DEBUG;  //PAQUETs
    int i=0;
    printf("%d:{ ",c->node);
    for(i=0;i<list_NES_taille(nodedata->paquets);i++)    printf("(%d,%d),",list_NES_get(nodedata->paquets,i).source,list_NES_get(nodedata->paquets,i).seq);
    printf("} nbrv: %d\n",list_NES_taille(nodedata->paquets));//*/
