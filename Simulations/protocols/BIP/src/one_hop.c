...
int init_one_hop(call_t *c, void *args) {
    ...
    packet_t *packet = packet_create(c, nodedata->overhead + sizeof(struct packet_hello), -1);
...
int rx_one_hop(call_t *c, packet_t *packet) {
	...
	//creation de paquet et initialisation de son data
	packet_t *rpacket = packet_create(c, nodedata->overhead + sizeof(struct packet_hello), -1);
...
void rx_one_hop_reponse(call_t *c, packet_t *packet) {
    ...
    if(!listeNodes_recherche(nodedata->N1,data->source))
        listeNodes_insert_values(&nodedata->N1,data->source,data->position.x, data->position.y,data->position.z);
