...
\begin{figure}[tb]
    \centering
    \begin{tabular}{cccc}
      \includegraphics[angle=90, scale= 0.5,width=.3\linewidth]{Etat_de_l'art/source/GrapheUnit.pdf} &
      \includegraphics[angle=90, scale= 0.5,width=.3\linewidth]{Etat_de_l'art/source/GrapheRNG.pdf} &
      \includegraphics[angle=90, scale= 0.5,width=.3\linewidth]{Etat_de_l'art/source/GrapheMST.pdf} 
      \\
      (a) & (b) & (c) \\
    \end{tabular}
    \caption{Graphe unité(a), graphe RNG(b) et graphe MST(c) \label{fig:ex}}
\end{figure}
...
\emph{Exemple :} % TODO : exemple + conso
...
\emph{Exemple :} %TODO : exemple, consommation
...
\subsubsection{Localized Broadcast Oriented Protocol(LBOP) et  RNG Broadcast Oriented Protocol(RBOP) \cite{Cartigny2005}}
\emph{Mots clés :} broadcast, local,LMST, RNG, 1-voisinage, rayon d'émission variable, déterministe.

\emph{Description :} LBOP et RBOP sont des protocole de diffusion s'appuyant respectivement sur l'abre couvrant de cout minimum local(LMST) et le graphe de voisinage(RNG).\\
Le graphe RNG peut etre obtenue raidement de façon locale et necessite la connaissance du 1-voisinage. Etant donné que chaque site connait sa position et celle de son 1-voisinage, il peut construire son graphe RNG
sur celui-ci. La reunion des tout ses sous graphe est exactement RNG(G).Une fois ce graphe construit, le message est diffusé via celui-ci, chaque noeud  transmettant à 'ses voisins les plus proche' n'ayant pas encore reçu le message.\\



LMST est la version distribués pour calculer un arbtre couvrant de cout minimum. Cepandant LMST n'est pas forcement de cout minimum. Pour calculer le graphe LMST, chaque site $u$ construit $LMST(N_1(u))$:\\
 $(u,v)\in LMST(N_1(u)) \Leftrightarrow v \in N_{MST(N_1(u))}(u)$ et $ u \in N_{MST(N_1(v))}(v)$\\
Avec $N_{MST(N_1(u))}(v)$ les voisins de $v$ dans l'abre couvrant calculeé sur son 1-voisinage(avec PRIM par exemple).\\
Une fois l'arbre construit, le message est diffusé via celui-ci, chaque noeud pere transmettant à ses fils.
...
En remplacant $RNG(u)$ par $LMST(u)$ dans l'algorithme de RBOP, nous obtenons LBOP.\\

\emph{Consommation :}En pratique, étant donnée que 
\emph{Durée de vie :} 
...
\emph{Description :} TRLBOP est une amélioration de LBOP. Le principe de choisir les noeuds relais pour qu'ils se rapproche le plus possible d'un maillage
 hexagonale de rayon optimal théorique $R_{opt}=\sqrt[\alpha]{\frac{2c}{\alpha-2}}$.Au lieu d'envoyer le message $M$ avec pour rayon $\max\limits_{w\in N_{LMST(u)}}(d_e(u,w))$, le rayon choisie est la distance entre $u$ et un voisins non
encore couvert par le broadcast qui est la plus proche de $R_{opt}$ . \\
Soient  $L(u)= N_{LMST}(u)$ et $L'(u)=N_1(u)\backslash L(u)$.\\
Soient  $D_L=\max(d_e(u,v)\mid v \in L(u))$ et \\ $D_{L'}=(d_e(u,v)\mid v \in L(u)\cup L'(u)\wedge \delta_{uv}=min(\delta_{uv}\mid w \in L(u)\cup L'(u)))$ avec $\delta_{uv}=\mid d_e(u,v)-R_{opt}\mid$.\\
...
%>>>>>>> 1893df8ee14e584385a58441d27c3c8189d12db0
\hline
\end{tabular}
}
}
